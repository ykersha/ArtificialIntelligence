...
//	}
	...
	static class Checker {
		...
		byte a; //m
		byte b; //n
		HashMap<String, Byte> ss = new HashMap<String, Byte>(); //ships
	...
	public static boolean applyPlan(String grid, String solution) {
		...
		int blue = Integer.parseInt(solutionArray[1]); //deaths
		int doors = Integer.parseInt(solutionArray[2]); //retrieved
		...
		byte x = Byte.parseByte(gridArray[1]); //capacity

		String[] xx = gridArray[2].split(","); 
		byte x00 = Byte.parseByte(xx[0]);	//CGx
		byte x01 = Byte.parseByte(xx[1]);	//CGY

		String[] st = gridArray[3].split(",");
		ArrayList<String> xyz = new ArrayList<String>(); // xyz = stations
		for (int i = 0; i < st.length - 1; i += 2) {
			xyz.add(st[i] + "," + st[i + 1]);
		}

		String[] sh = gridArray[4].split(",");
		HashMap<String, Byte> m4 = new HashMap<String, Byte>(); //m4 == ships
